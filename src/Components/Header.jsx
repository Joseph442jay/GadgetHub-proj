      ...
      <div className="flex relative gap-2">
        ...
        {isOpen && (
          ...
          >
            ...
            <button onClick={handleLogout} className="text-[#E60E0E] text-left">
              Logout
            </button>
          </motion.div>
        )}
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="container mx-auto flex items-center justify-between gap-0 lg:gap-25 h-[100px] py-10 lg:py-2 px-5">
        ...
        <Link to="/">
          <img
            className="w-[152px] h-[39px] hidden lg:flex"
            src={Logo}
            alt=""
          />
        </Link>
        ...
        <form
          ...
          className="hidden h-12 lg:flex relative lg:w-[556px]"
        >
          <Input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            type="text"
            className="w-full h-full rounded-md border-[#ACACAC]"
              ...
              {icons.map((icon) =>
                icon.id === 1 ? (
                  ...
                  <Button
                    ...
                    className="w-[84px] h-[48px] text-white"
                    content="Login"
                  />
                ),
              )}
          ...
          {menuOpen && (
            ...
            >
              <button onClick={toggleMenu} className="lg:hidden text-2xl">
                {menuOpen ? <IoCloseSharp size={30} /> : <GiHamburgerMenu />}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="flex bg-[#191C1F] text-white mt-1">
        <div className="hidden lg:flex items-center container mx-auto h-[65px] text-sm lg:text-lg px-5 gap-8">
          {filters.map((filter) => {
            return (
              <Link key={filter.id} to={filter.linkTo}>
                {filter.name}
              </Link>
            );
          })}
